...
| **AI**         | Ollama local (default), no key needed     | providers wired but dormant (no calls)   | Gemini (`GEMINI_API_KEY` from env)      |
...
> The AI **provider** is chosen per capability by `spring.ai.model.chat`/`embedding` — `dev` defaults to
> local **Ollama** (no key), `prod` to **Gemini** (key from env). The beans are lazy, and nothing calls a
> model until `discovery` is built, so the app boots without any AI credential. Only the pgvector store
> autoconfig stays excluded for now. See [LOCAL_AI.md](LOCAL_AI.md).
...
- AI defaults to **local Ollama** (`localhost:11434`) for chat + embeddings — no cloud key needed.
  To test cloud instead, override the switch (e.g. `SPRING_AI_MODEL_CHAT=google-genai` + `GEMINI_API_KEY`).

...
public enum WorkspaceError implements ErrorCatalog {
    ...
    MEMBER_PLAN_LIMIT_EXCEEDED(HttpStatus.UNPROCESSABLE_CONTENT),
    PROJECT_ROLE_NOT_FOUND(HttpStatus.NOT_FOUND),
    PROJECT_ROLE_NAME_ALREADY_EXISTS(HttpStatus.CONFLICT),
    PROJECT_ROLE_IN_USE(HttpStatus.CONFLICT),
    PROJECT_MEMBER_NOT_FOUND(HttpStatus.NOT_FOUND),
    PROJECT_MEMBER_ALREADY_EXISTS(HttpStatus.CONFLICT),
    ORGANIZATION_OWNER_CANNOT_LEAVE(HttpStatus.CONFLICT),
    INVALID_OWNERSHIP_TRANSFER(HttpStatus.CONFLICT),
    ...
    MEMBER_NOT_PENDING(HttpStatus.CONFLICT);

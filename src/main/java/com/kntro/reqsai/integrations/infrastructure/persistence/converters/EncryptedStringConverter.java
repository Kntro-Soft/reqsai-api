package com.kntro.reqsai.integrations.infrastructure.persistence.converters;

import com.kntro.reqsai.integrations.infrastructure.crypto.AesGcmCipher;
 ...
 * <p>
 * JPA converters are instantiated by Hibernate, not Spring, so the {@link AesGcmCipher} is supplied
...
public class EncryptedStringConverter implements AttributeConverter<String, byte[]> {
    ...
    private static volatile @Nullable AesGcmCipher cipher;

    /** Wired once at startup by the crypto configuration. */
    public static void setCipher(AesGcmCipher aesGcmCipher) {
        cipher = aesGcmCipher;
    }

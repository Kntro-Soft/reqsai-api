package com.kntro.reqsai.discovery.infrastructure.ai.transcription.batch.config;

import com.kntro.reqsai.discovery.application.port.TranscriptionPort;
import com.kntro.reqsai.discovery.infrastructure.ai.transcription.batch.SttRouter;
...
/**
 * Wires the batch STT infrastructure: instantiates each provider adapter and registers an
 * {@link SttRouter} as the single {@link TranscriptionPort} bean — unless a test (or another module)
 * already provided one ({@code @ConditionalOnMissingBean} on a {@code @Bean} method is the supported
 * Spring Boot pattern).
 */
@Configuration
class TranscriptionConfiguration {
    ...
    TranscriptionPort sttRouter(
            ...
            @Value("${reqsai.ai.stt.batch.provider:whisper}") String provider,

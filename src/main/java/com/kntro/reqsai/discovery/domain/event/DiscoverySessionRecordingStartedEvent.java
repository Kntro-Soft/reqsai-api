...
/**
 * Raised when a session transitions from {@code DRAFT} to {@code RECORDING}. Carries the session's
 * descriptive fields so project-level realtime consumers can render the update without a DB round-trip.
 */
public record DiscoverySessionRecordingStartedEvent(
    ...
    public static DiscoverySessionRecordingStartedEvent of(UUID sessionId, UUID projectId,
        ...
        return new DiscoverySessionRecordingStartedEvent(sessionId, projectId, title, language, startedAt, Instant.now());
    }
